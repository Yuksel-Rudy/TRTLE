...
import pandas as pd
import os
dataFile = os.path.join(os.path.dirname(os.path.abspath(__file__)), "CpCt_curves_IEA15_FLORIS.xlsx")
# dataFile = os.path.join(os.path.dirname(os.path.abspath(__file__)), "CpCt_curves_IEA15.xlsx")
data = pd.read_excel(dataFile)
v = data['Wind [m/s]']
# cp = data['Power Coefficient [-]']
power = data['Power [kW]']
ct = data['Thrust Coefficient [-]']
...
# cp[v > 10.8] *= 0.9880837  # correction to get exact 15.00MW
...
diameter = 242.24
# power = 1/2 * cp * 1.225 * (np.pi * (diameter/2)**2) * v**3 / 1e3  # kW
...
def main():
    ...
    print('Rated power [kW]', wt.power(wt.rated_wind_speed)/1e3)
    ws = np.arange(3, 25)
